...
export default function StockDetailsModal({ open, stock, onClose }: Props) {
  ...
  const [tradeSkeletonCount, setTradeSkeletonCount] = useState(0);
  ...
  const fetchPortfolios = useCallback(async () => {
    ...
    try {
      ...
      setTradeSkeletonCount(activePortfolios.length);
          ...
          <div className="flex flex-1 min-h-0 flex-col lg:flex-row overflow-hidden">
            ...
            >
              ...
              <div className="flex-1 min-h-0 overflow-y-auto space-y-2 pr-3">
                {loading ? (
                  Array.from({
                    length: tradeSkeletonCount > 0 ? tradeSkeletonCount : 3,
                  }).map((_, index) => (
                    <div
                      key={`portfolio-skeleton-${index}`}
                      className="bg-white border border-gray-200 rounded-md p-2 pl-3 flex items-center justify-between gap-2 animate-pulse"
                    >
